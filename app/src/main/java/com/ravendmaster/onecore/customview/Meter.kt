        ...
        try {
            ...
            for (i in 0 until numberOfMarks) {
                ...
                if (phys_arrow_value >= i) {
                    p.color = currentIndicationColor
                } else {
                    p.color = 0x505050
                }

                val x=i * button_width / (numberOfMarks - 1)
            ...
            if (label != null) {
                ...
                if(phys_arrow_value>0 && active_color==MyColors.getRed()) {
                    var pan_x = button_width / 2 - (bounds.right - bounds.left) / 2;
                    var pan_y = button_height / 2 - (bounds.bottom - bounds.top) / 2;
                    var over = meter_height / 12;
                    p.setColor(0xffffff);
